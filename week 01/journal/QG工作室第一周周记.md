...
- #### 继续写学生登录
  ...
  1.  查看报修单。这里使用mybatisplus的selectList得到所有特定的列表。全部列表直接传入null，特定就使用QueryWrapper<>进行“eq”,”gt”,”like”的确定用户范围。得到的列表判定isEmpty就返回，不然就for ()遍历信息。
  1.  取消报修单。同样的先用selectList和QueryWrapper<>获取所有的该学生的报修表，然后展示所有active的报修单。输入要修改的报修单的单号后，用selectById找到该报修单，接着设置边界条件，再updateById更新状态
  1.  修改密码。实际上就是确认两遍新密码后，把新密码set到该学生的password，接着updateById即可。
  
  

- #### 跑一遍数据

​		刚开始跑的时候，报红一大堆。先是数据库名搞错了，改对了后列名系统又无法识别，最后所有有大小写的列名全部都添加了一层@TableField()。后来知道直接添加个“驼峰映射”的依赖就可以了。

​		不报红了后，我随便输入了些数据，想看看边界条件做的怎么样。给提示是给了，但系统自带的日志几乎掩盖了提示，我想修复，但查了查，还是以后学到前端做前后端连接的项目再搞吧。

​		边界条件处理是有些问题的，当时设置了一些”不可输入空白“的条件，用if(sc.hasNext())，但实际不会进入else里面，会阻塞在true与false之间，等待用户输入下一个非空白字段。最后改成了先输入再equal判断空白。
...
​		*早上十点多起，按照写学生的逻辑很快就把管理员的写完了。*

- #### 完成管理员登录

​		这里不详细写怎么实现的了，原理都和学生的差不多。幸亏昨晚先测试了下学生的代码，可以避免很多的bug。根据朋友的建议，我在resourse加入了tables.sql文件，记录了是如何创建数据库里的表的。



- #### 进阶

​		忘记在报修单加入时间属性了，待会补上。还想添加个加密与管理员评价。感觉有点麻烦，等下周吧。
