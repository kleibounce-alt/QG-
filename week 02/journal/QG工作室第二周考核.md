...
- #### 前端后端整个流程
  ...
  - **注册：**
    - 用户点击注册后，register.vue调用api，api确定接口地址与请求方式，利用axios封装数据以json格式往后台具体接口发送。controller收到后，进入service，把数据存入数据库后，返回状态码。
  - **登录：**
    - 用户点击登录后，login.vue调用api，api调用axios封装后，发往后台，controller接收后，进入service调用数据库校验账号密码，检查无误后，jwt生成token，返回状态码与token，token在前端被本地储存。
  - **登录后续：**
    - 用户点击后，某个vue调用api，再用axios封装，此时axios自动添加token请求头，发给后台，后续步骤与其他基本一致。



- #### 哪些操作会触发请求

  - **点击、提交、选择、加载、滚动、输入、定时** 
